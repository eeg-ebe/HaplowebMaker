...
mj_MJAlgo.prototype = {
	...
	,countHeterozygousIndiv: function() {
		var seenNames = new haxe_ds_StringMap();
		var current = this.seqs.first;
		while(current != null) {
			var _g_head = current.indNames.h;
			while(_g_head != null) {
				...
				_g_head = _g_head.next;
				var s = val;
				var count = __map_reserved[s] != null ? seenNames.getReserved(s) : seenNames.h[s];
				if(count == null) {
					count = 0;
				}
				++count;
				if(__map_reserved[s] != null) {
					seenNames.setReserved(s,count);
				} else {
					seenNames.h[s] = count;
				}
			}
		...
		var count1 = 0;
		var key = seenNames.keys();
		while(key.hasNext()) {
			var key1 = key.next();
			if((__map_reserved[key1] != null ? seenNames.getReserved(key1) : seenNames.h[key1]) >= 2) {
				++count1;
			}
		}
		return count1;
