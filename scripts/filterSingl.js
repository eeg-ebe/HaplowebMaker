...
FilterSingletons.checkMaybeMissing = function(faFile,delimiter) {
	var countingMap = new haxe_ds_StringMap();
	var indNameToSeq = new haxe_ds_StringMap();
	var _g_head = faFile.h;
	while(_g_head != null) {
		var val = _g_head.item;
		_g_head = _g_head.next;
		var seq = val;
		var header = seq.first;
		var result = header;
		if(header != null) {
			var pos = header.lastIndexOf(delimiter);
			if(pos != -1) {
				result = HxOverrides.substr(header,0,pos);
			}
		}
		var indName = result;
		if(indName != header) {
			if(__map_reserved[indName] != null) {
				indNameToSeq.setReserved(indName,header);
			} else {
				indNameToSeq.h[indName] = header;
			}
			if(__map_reserved[indName] != null ? countingMap.existsReserved(indName) : countingMap.h.hasOwnProperty(indName)) {
				var count = __map_reserved[indName] != null ? countingMap.getReserved(indName) : countingMap.h[indName];
				++count;
				if(__map_reserved[indName] != null) {
					countingMap.setReserved(indName,count);
				} else {
					countingMap.h[indName] = count;
				}
			} else if(__map_reserved[indName] != null) {
				countingMap.setReserved(indName,1);
			} else {
				countingMap.h[indName] = 1;
			}
		}
	}
	var result1 = new List();
	var key = countingMap.keys();
	while(key.hasNext()) {
		var key1 = key.next();
		if((__map_reserved[key1] != null ? countingMap.getReserved(key1) : countingMap.h[key1]) <= 1) {
			result1.add(__map_reserved[key1] != null ? indNameToSeq.getReserved(key1) : indNameToSeq.h[key1]);
		}
	}
	return result1.join(", ");
};
