...
FilterSingletons.checkMaybeMissing = function(faFile,delimiter) {
	...
	while(_g_head != null) {
		...
		if(__map_reserved[indName] != null ? map.existsReserved(indName) : map.h.hasOwnProperty(indName)) {
			var count = __map_reserved[indName] != null ? map.getReserved(indName) : map.h[indName];
			++count;
			if(__map_reserved[indName] != null) {
				map.setReserved(indName,count);
			} else {
				map.h[indName] = count;
			}
	...
	while(key.hasNext()) {
		var key1 = key.next();
		if((__map_reserved[key1] != null ? map.getReserved(key1) : map.h[key1]) <= 1) {
			result1.add(key1);
		}
	}
	return result1.join(", ");
